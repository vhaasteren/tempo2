...
/*                                                                  */
/* TODO(quickplot-maintenance):                                     */
/* - Revisit remaining quickplot/plk behavior mismatches after the  */
/*   SWGP and selector-coordinate consistency pass.                 */
/* - Consolidate shared menu geometry, hit-testing, and plot-mode   */
/*   mapping helpers to avoid future drift between plugins.         */
/* - Long-term: merge plk and quickplot into one robust,            */
/*   well-structured visualization plugin with shared tested logic. */
 ...
 * spacing, or add a new selector entry, update quickplot_axis_selector_buttons
 * and the QUICKPLOT_AXIS_SELECTOR_* / QUICKPLOT_MENU3_VIEWPORT_* constants here
 * rather than editing drawMenu3() or checkMenu3() directly.
 */
struct QuickplotAxisSelectorButton {
    float y_top;
    int plot;
    const char *label;
};
static const float QUICKPLOT_MENU3_VIEWPORT_X_MAX = 0.3f;
static const float QUICKPLOT_MENU3_VIEWPORT_Y_MIN = 0.3f;
static const float QUICKPLOT_MENU3_VIEWPORT_Y_SPAN = 0.5f;
static const float QUICKPLOT_AXIS_SELECTOR_X_COLUMN_MIN = 0.50f;
static const float QUICKPLOT_AXIS_SELECTOR_X_COLUMN_MAX = 0.55f;
static const float QUICKPLOT_AXIS_SELECTOR_Y_COLUMN_MIN = 0.60f;
static const float QUICKPLOT_AXIS_SELECTOR_Y_COLUMN_MAX = 0.65f;
static const float QUICKPLOT_AXIS_SELECTOR_BUTTON_HEIGHT = 0.04f;
static const QuickplotAxisSelectorButton quickplot_axis_selector_buttons[] = {
    {1.00f, 1, "pre-fit"},
    {0.94f, 2, "post-fit"},
    {0.88f, 3, "date"},
    {0.82f, 4, "orbital phase"},
    {0.76f, 5, "serial"},
    {0.70f, 6, "day of year"},
    {0.64f, 7, "frequency"},
    {0.58f, 8, "TOA error"},
    {0.52f, 10, "year"},
    {0.46f, 11, "elevation"},
    {0.39f, 12, "rounded MJD"},
    {0.34f, 13, "sidereal time"},
    {0.29f, 14, "hour angle"},
    {0.24f, 15, "para. angle"},
    {0.19f, 16, "Red Noise"},
    {0.14f, 17, "DM Var"},
    {0.09f, 20, "SWGP"},
    {0.04f, 19, "Chrom. Noise"}
};
static bool quickplotAxisSelectorEnabled(const pulsar *psr, int plot)
{
    switch (plot) {
        case 16:
...
static int quickplotAxisSelectorHitPlot(const pulsar *psr, float menu_x, float menu_y)
{
    if (menu_x < QUICKPLOT_AXIS_SELECTOR_X_COLUMN_MIN || menu_x > QUICKPLOT_AXIS_SELECTOR_Y_COLUMN_MAX)
        return -1;
    for (unsigned idx = 0; idx < sizeof(quickplot_axis_selector_buttons)/sizeof(quickplot_axis_selector_buttons[0]); ++idx) {
        const QuickplotAxisSelectorButton &button = quickplot_axis_selector_buttons[idx];
        if (!quickplotAxisSelectorEnabled(psr, button.plot))
            continue;
        if (menu_y <= button.y_top && menu_y >= button.y_top - QUICKPLOT_AXIS_SELECTOR_BUTTON_HEIGHT)
            return button.plot;
    }
    return -1;
}
        ...
        char highlightID[100][100],char  highlightVal[100][100],int *highlightNum,float aspect,int fontType,int lineWidth,char *bkgrdColour,char *lineColour,int *jumpOffset) 
{
    ...
    y7 = (my-y3)*yscale+y1; 
    {
        ...
        const float menu_x = xfrac / QUICKPLOT_MENU3_VIEWPORT_X_MAX;
        const float menu_y = (yfrac - QUICKPLOT_MENU3_VIEWPORT_Y_MIN) / QUICKPLOT_MENU3_VIEWPORT_Y_SPAN;
        int axis_col = -1;
        int axis_plot = -1;

        if (menu_x >= QUICKPLOT_AXIS_SELECTOR_X_COLUMN_MIN && menu_x <= QUICKPLOT_AXIS_SELECTOR_X_COLUMN_MAX) axis_col = 0;
        else if (menu_x >= QUICKPLOT_AXIS_SELECTOR_Y_COLUMN_MIN && menu_x <= QUICKPLOT_AXIS_SELECTOR_Y_COLUMN_MAX) axis_col = 1;
        if (axis_col != -1)
            axis_plot = quickplotAxisSelectorHitPlot(psr, menu_x, menu_y);
